...
namespace Com
{
    ...
    public struct PointD6D : IEquatable<PointD6D>, IComparable, IComparable<PointD6D>, IEuclideanVector<PointD6D>, IAffineTransformable<PointD6D>, IAffine<PointD6D>
    {
        ...
        public override string ToString() => string.Concat("{X=", _X, ", Y=", _Y, ", Z=", _Z, ", U=", _U, ", V=", _V, ", W=", _W, "}");
        ...
        public PointD6D MatrixTransformCopy(Matrix matrixLeft)
        {
            Vector result = ToColumnVector().MatrixTransformCopy(matrixLeft);
        ...
        public PointD6D AffineTransformCopy(AffineTransformation affineTransformation)
        {
            Vector result = ToColumnVector().AffineTransformCopy(affineTransformation);
        ...
        public Vector ToColumnVector() => Vector.UnsafeCreateInstance(Vector.Type.ColumnVector, _X, _Y, _Z, _U, _V, _W);

        /// <summary>
        /// 返回将此 PointD6D 结构转换为行向量的 Vector 的新实例。
        /// </summary>
        /// <returns>Vector 对象，表示将此 PointD6D 结构转换为行向量得到的结果。</returns>
        public Vector ToRowVector() => Vector.UnsafeCreateInstance(Vector.Type.RowVector, _X, _Y, _Z, _U, _V, _W);
        ...
        public static Matrix OffsetMatrix(double dx, double dy, double dz, double du, double dv, double dw) => Vector.OffsetMatrix(Vector.UnsafeCreateInstance(Vector.Type.ColumnVector, dx, dy, dz, du, dv, dw));
        ...
        public static Matrix ScaleMatrix(double sx, double sy, double sz, double su, double sv, double sw) => Vector.ScaleMatrix(Vector.UnsafeCreateInstance(Vector.Type.ColumnVector, sx, sy, sz, su, sv, sw));
        ...
        public static implicit operator PointD6D((double, double, double, double, double, double) tuple)
        {
            return new PointD6D(tuple.Item1, tuple.Item2, tuple.Item3, tuple.Item4, tuple.Item5, tuple.Item6);
        }
