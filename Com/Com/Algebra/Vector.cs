...
namespace Com
{
    ...
    public sealed class Vector : IEquatable<Vector>, IComparable, IComparable<Vector>, IEuclideanVector<Vector>, ILinearAlgebraVector<Vector>, IAffineTransformable<Vector>, IAffine<Vector>
    {
        ...
        private static readonly double[] _EmptyData = Array.Empty<double>(); // 表示 Empty 对象在各基向量方向的分量的数组。

        //

        // 获取指定向量类型与维度的零向量。
        private static Vector _GetZeroVector(VectorType type, int dimension)
        {
        ...
        internal static Vector UnsafeCreateInstance(VectorType type, params double[] values)
        {
        ...
        private VectorType _Type; // 此 Vector 的向量类型。
        ...
        public Vector(VectorType type, params double[] values)
        {
            _Type = type;
        ...
        /// <param name="type">向量类型。</param>
        /// <param name="values">各基向量方向的分量。</param>
        [Obsolete("请改为使用 Vector(VectorType, params double[]) 构造方法")]
        public Vector(Type type, params double[] values) : this((VectorType)type, values) { }
        ...
        /// </summary>
        public bool IsColumnVector => _Size > 0 && _Type == VectorType.ColumnVector;

        /// <summary>
        /// 获取表示此 Vector 是否为行向量的布尔值。
        /// </summary>
        public bool IsRowVector => _Size > 0 && _Type == VectorType.RowVector;
        ...
        public Vector Transport
        {
            get
            {
                ...
                {
                    return new Vector(_Type == VectorType.ColumnVector ? VectorType.RowVector : VectorType.ColumnVector, _VArray);
                }
            }
        }
        ...
        public override string ToString()
        {
            ...
            {
                str = $"Type={(_Type == VectorType.ColumnVector ? "ColumnVector" : "RowVector")}, Dimension={_Size}";
            }
        ...
        public Matrix ToMatrix()
        {
            ...
            {
                if (_Type == VectorType.ColumnVector)
                {
        ...
        public void Shear(int index1, int index2, double angle)
        {
            ...
            {
                throw new IndexOutOfRangeException(nameof(index1));
            }
        ...
        public static Vector Zero(VectorType type, int dimension)
        {
            if (dimension < 0)
            {
                throw new OverflowException();
        ...
        /// <param name="dimension">向量维度。</param>
        /// <returns>Vector 对象，表示零向量。</returns>
        [Obsolete("请改为使用 Zero(VectorType, int) 方法")]
        public static Vector Zero(Type type, int dimension) => Zero((VectorType)type, dimension);
        ...
        /// <param name="dimension">向量维度。</param>
        /// <returns>Vector 对象，表示零向量。</returns>
        public static Vector Zero(int dimension) => Zero(VectorType.ColumnVector, dimension);
        ...
        public static Vector Base(VectorType type, int dimension, int index)
        {
            if (dimension < 0)
            {
                throw new OverflowException();
        ...
        /// <param name="index">索引。</param>
        /// <returns>Vector 对象，表示指定索引的基向量。</returns>
        [Obsolete("请改为使用 Base(VectorType, int, int) 方法")]
        public static Vector Base(Type type, int dimension, int index) => Base((VectorType)type, dimension, index);
        ...
        /// <param name="index">索引。</param>
        /// <returns>Vector 对象，表示指定索引的基向量。</returns>
        public static Vector Base(int dimension, int index) => Base(VectorType.ColumnVector, dimension, index);
        ...
        public static Matrix OffsetMatrix(VectorType type, int dimension, int index, double d)
        {
            if (dimension < 0)
            {
                throw new OverflowException();
        ...
        /// <param name="d">双精度浮点数表示的位移。</param>
        /// <returns>Matrix 对象，表示按双精度浮点数表示的位移将 Vector 对象在指定的基向量方向的分量平移指定的量的仿射矩阵。</returns>
        [Obsolete("请改为使用 OffsetMatrix(VectorType, int, int, double) 方法")]
        public static Matrix OffsetMatrix(Type type, int dimension, int index, double d) => OffsetMatrix((VectorType)type, dimension, index, d);
        ...
        public static Matrix OffsetMatrix(VectorType type, int dimension, double d)
        {
            if (dimension < 0)
            {
                throw new OverflowException();
        ...
        /// <param name="d">双精度浮点数表示的位移。</param>
        /// <returns>Matrix 对象，表示按双精度浮点数表示的位移将 Vector 对象的所有分量平移指定的量的仿射矩阵。</returns>
        [Obsolete("请改为使用 OffsetMatrix(VectorType, int, double) 方法")]
        public static Matrix OffsetMatrix(Type type, int dimension, double d) => OffsetMatrix((VectorType)type, dimension, d);
        ...
        public static Matrix OffsetMatrix(Vector vector)
        {
            ...
            else
            {
                ...
                if (vector._Type == VectorType.ColumnVector)
                {
                    for (int i = 0; i < dimension; i++)
                    {
        ...
        public static Matrix ScaleMatrix(int dimension, int index, double s)
        {
            if (dimension < 0)
            {
                throw new OverflowException();
        ...
        /// <param name="s">双精度浮点数表示的缩放因数。</param>
        /// <returns>Matrix 对象，表示按双精度浮点数表示的缩放因数将 Vector 对象在指定的基向量方向的分量缩放指定的倍数的仿射矩阵。</returns>
        [Obsolete("请改为使用 ScaleMatrix(int, int, double) 方法")]
        public static Matrix ScaleMatrix(Type type, int dimension, int index, double s) => ScaleMatrix(dimension, index, s);
        ...
        public static Matrix ScaleMatrix(int dimension, double s)
        {
            if (dimension < 0)
            {
                throw new OverflowException();
        ...
        /// <param name="s">双精度浮点数表示的缩放因数。</param>
        /// <returns>Matrix 对象，表示按双精度浮点数表示的缩放因数将 Vector 对象的所有分量缩放指定的倍数的仿射矩阵。</returns>
        [Obsolete("请改为使用 ScaleMatrix(int, double) 方法")]
        public static Matrix ScaleMatrix(Type type, int dimension, double s) => ScaleMatrix(dimension, s);
        ...
        public static Matrix ReflectMatrix(int dimension, int index)
        {
            if (dimension < 0)
            {
                throw new OverflowException();
        ...
        /// <param name="index">索引，用于指定翻转的分量所在方向的基向量。</param>
        /// <returns>Matrix 对象，表示将 Vector 对象在指定的基向量方向的分量翻转的仿射矩阵。</returns>
        [Obsolete("请改为使用 ReflectMatrix(int, int) 方法")]
        public static Matrix ReflectMatrix(Type type, int dimension, int index) => ReflectMatrix(dimension, index);
        ...
        public static Matrix ShearMatrix(VectorType type, int dimension, int index1, int index2, double angle)
        {
            if (dimension < 0)
            {
                throw new OverflowException();
            ...
            if (index1 < 0 || index1 >= dimension)
            {
                throw new IndexOutOfRangeException(nameof(index1));
            }

            if (index2 < 0 || index2 >= dimension)
            {
                throw new IndexOutOfRangeException(nameof(index2));
            }
        ...
        /// <param name="angle">双精度浮点数，表示 Vector 对象沿索引 index1 指定的基向量方向且共面正交于 index2 指定的基向量方向错切的角度（弧度）。</param>
        /// <returns>Matrix 对象，表示按双精度浮点数表示的弧度将 Vector 对象错切指定的角度的仿射矩阵。</returns>
        [Obsolete("请改为使用 ShearMatrix(VectorType, int, int, int, double) 方法")]
        public static Matrix ShearMatrix(Type type, int dimension, int index1, int index2, double angle) => ShearMatrix((VectorType)type, dimension, index1, index2, angle);
        ...
        public static Matrix RotateMatrix(VectorType type, int dimension, int index1, int index2, double angle)
        {
            if (dimension < 0)
            {
                throw new OverflowException();
        ...
        /// <param name="angle">双精度浮点数，表示 Vector 对象绕索引 index1 与 index2 指定的基向量构成的平面的法向空间旋转的角度（弧度）（以索引 index1 指定的基向量为 0 弧度，从索引 index1 指定的基向量指向索引 index2 指定的基向量的方向为正方向）。</param>
        /// <returns>Matrix 对象，表示按双精度浮点数表示的弧度将 Vector 对象旋转指定的角度的仿射矩阵。</returns>
        [Obsolete("请改为使用 RotateMatrix(VectorType, int, int, int, double) 方法")]
        public static Matrix RotateMatrix(Type type, int dimension, int index1, int index2, double angle) => RotateMatrix((VectorType)type, dimension, index1, index2, angle);
        ...
        private sealed class Enumerator : IEnumerator // 实现 System.Collections.IEnumerator 的迭代器。
        {
            ...
            object IEnumerator.Current
            {
                get
                {
                    if (_Vector is null)
                    {
                        throw new ArgumentNullException(nameof(_Vector));
                    }

                    if (_Index < 0 || _Index >= _Vector._Size)
                    {
